...
var needRedirect = function (req) {
  ...
  var isInternal = _.any(['/api/', '/forks/', '/views/', '/images/', '/bower_components/', '/scripts/', '/styles/', '/index.html', '/favicon.ico', '/browser-sync'],
    function (path) {
      return parsedUrl.pathname.indexOf(path) === 0
    });
  return !isInternal;
};
...
function browserSyncInit(baseDir, files, port, browser) {
  ...
  browserSync({
    ...
    server: {
      baseDir: baseDir,
      // 用户自定义的middleware优先
      middleware: (env.config.middlewares || []).concat([delayMiddleware, rewriteMiddleware, proxyMiddleware, forkMiddleware, historyApiMiddleware])
    },
    browser: browser,
    port: port,
    open: false
