...
gulp.task('copyLibraries', function () {
  ...
  var files = _.map(plugin.mainBowerFiles(), function (file) {
    return file.replace(env.folders.library, '');
  });
...
gulp.task('copyPartials', function () {
  return gulp.src([env.folders.app + '/partials/**/*.html'])
    .pipe(plugin.plumber())
    ...
    .pipe(gulp.dest(env.folders.build + '/partials'));
});

gulp.task('copyViews', ['copyPartials'], function () {
...
gulp.task('buildHome', function () {
  return gulp.src(env.folders.app + '/*.html')
    .pipe(plugin.plumber())
    .pipe(plugin.usemin({
      css: [plugin.minifyCss(), 'concat', rev()],
      html: [plugin.minifyHtml({empty: true, comments: true})],
      js: [plugin.ngAnnotate(), plugin.uglify({preserveComments: plugin.uglifySaveLicense}), plugin.rev()]
    }))
    .pipe(gulp.dest(env.folders.build));
});
...
gulp.task('build', function (done) {
  plugin.runSequence('compile', 'purge', 'copyForks', 'copyLibraries', 'copyFonts', 'copyImages', 'copyViews',
